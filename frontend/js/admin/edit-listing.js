...
function handleListing(listing) {
    ...
    keys.forEach(key => {
        ...
        } else if (key == "Completion Month") {
            enterCompletionMonth(listing[key])
	    } else {
...
function enterCompletionMonth(value) {
    const select = document.querySelector("select[name='Completion Month']");
    select.value = value;
}
...
async function organizeData() {
    ...
    const completionMonth = document.querySelector(`select[name="Completion Month"]`).value;
    showLoadingPage();
    ...
    formdata.set("Completion Month", completionMonth)
